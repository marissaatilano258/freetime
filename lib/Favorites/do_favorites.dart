...
import 'package:free_time/Do/do_db_worker.dart';
import 'package:free_time/Do/do_model.dart';
...
class DoFavorites extends StatefulWidget {
  DoFavorites({Key key}) : super(key: key);

  @override
  _DoFavoritesState createState() => _DoFavoritesState();
}

class _DoFavoritesState extends State<DoFavorites> {
  ...
  void loadData() async {
    doList.clear();
    doList.addAll(await DoModelsDBWorker.db.getAll());
    setState(() {
    });
  }
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [
                  ...
                  color.AppColor.lightOrange.withOpacity(0.8),
                ],
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(
                                offset: Offset(5,5),
                                blurRadius: 5,
                                color: color.AppColor.lightOrange.withOpacity(0.6),
                              ),
                              BoxShadow(
                                offset: Offset(0,-1),
                                blurRadius: 1,
                                color: color.AppColor.lightOrange.withOpacity(0.2),
                              ),
                            ],
                          ),
                          ...
                          child: Slidable(
                            ...
                            secondaryActions: [
                              IconSlideAction(
                                  ...
                                  icon: Icons.delete,
                                  onTap: () => _deleteDo(context, i)
                              )
                            ],
                            child: ListTile(
                                title: Text(doList[i].name,
                                    style: TextStyle(fontSize: 18, fontWeight: FontWeight.w400)),
                                subtitle: Text('Location: ' + doList[i].location +
                                    '\nDate: ' + doList[i].date + ' ' + doList[i].time,
                                  ...
                                  children: [
                                    ...
                                    SizedBox(
                                        ...
                                        child: TextButton(
                                          ...
                                          onPressed: () async {
                                            await canLaunch(doList[i].website) ? await launch(doList[i].website) :
                                            showDialog<void>(
                                              context: context,
  ...
  _deleteDo(BuildContext context, int i) {
    return showDialog(
        context : context,
              ...
              actions : [
                ...
                TextButton(child : Text("Remove"),
                    onPressed : () async {
                      ...
                      await DoModelsDBWorker.db.delete(doList[i].id);
                      setState(() {
                        loadData();
                      });
                    }
